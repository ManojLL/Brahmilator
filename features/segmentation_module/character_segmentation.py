...
def get_blobs_sorted_by_distance_from_previous_blob(blobs, previous_blob):
    ...
    for blob in blobs:
        try:
            distance_blob_tuples.append((distance_between_points((int(blob.minx), int(blob.miny)),
                                                                 (int(previous_blob.maxx), int(previous_blob.miny))),
                                         blob))
        except:
...
def vp(blob, vp_dir, left_right_margin=10, show_blobs=True):
    ...
    if show_blobs:
        ...
        cvRectangle(blob_image_2, cvPoint(0, 0), cvPoint(blob_width, blob_height), CV_RGB(255, 255, 255), CV_FILLED, 8,
                    0)
...
def process_image(bi_image, folio_hp_directory, folio_vp_directory, folio_intermediate_directory,
                  folio_characters_directory, folio_name, num_lines, previous_blob_count=0):
    ...
    if need_to_rerun and initial_blob_count - previous_blob_count > 1:
        process_image(bi_image, folio_hp_directory, folio_vp_directory, folio_intermediate_directory,
                      folio_characters_directory, folio_name, num_lines, previous_blob_count=initial_blob_count)
    else:
        ...
        cvRectangle(black_square_image, cvPoint(0, 0), cvPoint(black_square_image.width, black_square_image.height),
        ...
        y_values = get_y_values_and_create_line_drawing(blobs, folio_intermediate_directory, line_drawing_image,
                                                        num_lines)
        y_bottom_values = create_line_drawing_from_black_squares(line_drawing_from_black_squares_image,
                                                                 black_square_image, folio_intermediate_directory,
                                                                 y_values)
        create_curved_bottom_line_drawing_image(curved_bottom_line_drawing_image, folio_intermediate_directory,
                                                y_bottom_values)
        ...
        for blob in blobs:
            ...
            blob_size = cvSize(blob_width, blob_height)
            blob_image = cvCreateImage(blob_size, 8, 1)
            cvRectangle(blob_image, cvPoint(0, 0), cvPoint(blob_image.width, blob_image.height), CV_RGB(255, 255, 255),
                        CV_FILLED, 8, 0)
...
if __name__ == "__main__":
    for folio_image_name in os.listdir(INPUT_IMAGE_DIR):
        try:
            folio_name = "%s_%s" % (
            datetime.datetime.now().strftime("Y%Y_m%m_d%d_H%H_M%M_S%S"), folio_image_name.rsplit('.')[0])
        ...
        if binary_image:
            process_image(binary_image, folio_hp_directory, folio_vp_directory, folio_intermediate_directory,
                          folio_characters_directory, folio_name, num_lines=8)
        else:
